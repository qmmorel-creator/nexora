...
/* Heures des tâches (#294, #299) : `startTime` porte sur la date `start`,
   `endTime` sur la date `end`, au format HH:MM, heure de Paris — les champs que
   lit la vue Calendrier de Nexora. Vide = tâche sur la journée entière. Même
   règle que la fiche tâche : sans fin, la tâche dure une heure ; une fin sans
   début, ou une fin avant le début le même jour, est refusée plutôt que
   corrigée en silence. */
export const TASK_TIME_RE=/^([01]\d|2[0-3]):([0-5]\d)$/;
export function resolveTaskTimes(task){
 const has=v=>v!=null&&String(v).trim()!=='';
 const toMin=v=>{const r=TASK_TIME_RE.exec(String(v).trim());return r?Number(r[1])*60+Number(r[2]):null;};
 if(!has(task.startTime)&&!has(task.endTime)){if('startTime'in task||'endTime'in task){task.startTime='';task.endTime='';}return task;}
 if(!has(task.startTime))throw new Error('endTime requires startTime (HH:MM)');
 const s=toMin(task.startTime);if(s==null)throw new Error('Invalid startTime: expected HH:MM (Europe/Paris)');
 let e=has(task.endTime)?toMin(task.endTime):null;if(has(task.endTime)&&e==null)throw new Error('Invalid endTime: expected HH:MM (Europe/Paris)');
 if(e==null)e=Math.min(s+60,1439);else if((task.end||task.start)===(task.start||task.end)&&e<=s)throw new Error('endTime must be after startTime on the same day');
 const fmt=m=>String(Math.floor(m/60)).padStart(2,'0')+':'+String(m%60).padStart(2,'0');
 task.startTime=fmt(s);task.endTime=fmt(e);return task;
}
...
export function filterTasks(tasks,catalogs,args) {
 ...
 return tasks.filter(t=>{
  ...
  if(args.focus!==undefined&&(t.focus===true)!==args.focus)return false;
...
export const querySchema={query:z.string().max(500).optional(),projectId:z.string().optional(),projectName:z.string().optional(),statusId:z.string().optional(),statusName:z.string().optional(),taskTypeId:z.string().optional(),taskTypeName:z.string().optional(),assignee:z.string().optional(),dateFrom:z.string().date().optional(),dateTo:z.string().date().optional(),dateField:z.enum(['start','end','overlap','completedAt','updatedAt']).default('end'),completed:z.boolean().optional(),meetingsOnly:z.boolean().optional(),hasReport:z.boolean().optional(),focus:z.boolean().optional().describe('Mode focus (#512) : true = tâches en focus seulement, false = hors focus. Absent = pas de filtre.'),sourceMessageId:z.string().optional(),sourceThreadId:z.string().optional(),documentId:z.string().optional(),googleEventId:z.string().optional(),archive:z.enum(['active','archived','all']).default('active'),sortBy:z.enum(['start','end','title','updatedAt']).default('end'),limit:z.number().int().min(1).max(100).default(40),cursor:z.string().max(1000).optional(),detail:z.enum(['summary','full']).default('summary')};
const jsonObject=z.record(z.unknown());
export const taskChangesSchema=z.object({title:z.string().min(1).max(240).optional(),description:z.string().max(100000).optional(),projectId:z.string().min(1).optional(),secondaryProjectId:z.string().nullable().optional(),statusId:z.string().min(1).optional(),taskTypeId:z.string().min(1).optional(),start:z.string().date().nullable().optional(),end:z.string().date().nullable().optional(),progress:z.number().min(0).max(100).optional(),assignee:z.string().max(500).optional(),priority:z.string().max(80).optional(),milestone:z.boolean().optional(),milestoneIcon:z.string().nullable().optional(),focus:z.boolean().optional().describe('Mode focus (#512) : tâche sur laquelle travailler absolument. Absent = false.'),checklist:z.array(jsonObject).max(500).optional(),dependsOn:z.array(z.string()).max(500).optional(),recurrence:jsonObject.nullable().optional(),customFields:jsonObject.optional(),attachments:z.array(jsonObject).max(200).optional(),source:z.string().optional(),sourceUrl:z.string().url().nullable().optional(),sourceMessageId:z.string().nullable().optional(),sourceThreadId:z.string().nullable().optional(),sourceSender:z.string().nullable().optional(),sourceReceivedAt:z.string().datetime({offset:true}).optional(),sourceReceivedDate:z.string().date().optional(),receivedDate:z.string().date().optional(),documentId:z.string().nullable().optional(),orderNumber:z.string().nullable().optional(),trackingNumber:z.string().nullable().optional(),startTime:z.union([z.literal(''),z.string().regex(TASK_TIME_RE,'HH:MM')]).nullable().optional().describe('Heure de début HH:MM, Europe/Paris, sur la date start. Vide ou null = toute la journée.'),endTime:z.union([z.literal(''),z.string().regex(TASK_TIME_RE,'HH:MM')]).nullable().optional().describe('Heure de fin HH:MM, Europe/Paris, sur la date end. Omise : une heure après startTime.'),location:z.string().optional(),participants:z.array(z.string()).optional()}).strict();
export function repository(db,uid){
 ...
 function enrich(t,c,full=false){const core=full?clean(t):Object.fromEntries(['id','title','projectId','secondaryProjectId','statusId','taskTypeId','start','end','startTime','endTime','progress','assignee','completedAt','sourceMessageId','sourceThreadId','documentId','googleEventId','gcalCalendarId','sourceUrl','focus','_archived'].filter(k=>t[k]!==undefined).map(k=>[k,t[k]]));const {_archived,...original}=t;return {...core,version:fingerprint(original),projectName:c.projects.find(x=>x.id===t.projectId)?.name??null,statusName:c.statuses.find(x=>x.id===t.statusId)?.name??null,taskTypeName:c.taskTypes.find(x=>x.id===t.taskTypeId)?.name??null,hasExplicitReport:reportEvidence(t,c.customFieldDefs).length>0,...(full?{reportEvidence:reportEvidence(t,c.customFieldDefs)}:{})};}
 ...
 async function mutateTask(args,action='update'){return atomic(action,args.idempotencyKey,args,async tx=>{
  ...
  if(action!=='delete'){ensureTaskDates(next);if(action==='update'&&['startTime','endTime','start','end'].some(k=>k in changes))resolveTaskTimes(next);}validateTask(next,c);next.updatedAt=new Date().toISOString();next.lastInteraction=next.updatedAt;
 ...
 async function createTask(args){return atomic('create_task',args.idempotencyKey,args,async tx=>{
  ...
  ensureTaskDates(task);resolveTaskTimes(task);validateTask(task,c);write(tx,d.tasks,[...c.tasks,task]);return {ok:true,created:true,task:clean(task),version:fingerprint(task)};
 });}
